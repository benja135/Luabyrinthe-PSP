...
-- Effet de particules, trajectoire sinusoïdale
...
local function create(i)
	particule[i] = 
	{
		x = math.random(0,479),
		y = math.random(0,271),
		dir = {a=math.random(50,200),p=math.random(10,100),o=math.random(80,120)}, -- "dir" pour direction, a=amplitude, p=période, o=ordonné
		sens = math.random(1,10), -- le sens des particules (on le détermine aprés pour choisir qu'entre 1 et -1)
		speed_alpha = math.random(1,20), -- vitesse d'alpha
		speed = math.random(0,1), -- vitesse de déplacement
		alpha = 255
	...
	if particule[i].sens > 5 then -- on détermine le sens
		particule[i].sens = 1
	else
		particule[i].sens = -1
	end
...
function particules(particules_max,death,size) 
	--particules(nombre de particules,particule éphémére(booléen),taille,une couleur ou 3 en fonction de la vitesse(booléen))
	
	-- on cré les particules qui manque
	...
	for i=1,particules_max do 
		...
		-- on calcule l'ordonnés des particule, le déplacement est sinusoïdale
